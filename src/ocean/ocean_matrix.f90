MODULE ocean_matrix
  ...
  ! =========================
    ...
    SUBROUTINE interpolation_with_d18O(mesh, ocean, time)
    ...
    SUBROUTINE apply_anomaly_fields(mesh, ocean, time)
    ...
    SUBROUTINE interpolation_with_insolation_and_GHG_radiative(mesh, ocean, time, grid_smooth)
    ...
    SUBROUTINE interpolation_with_insolation_and_GHG(mesh, ocean, time, grid_smooth)
      ! Linear interpolation forcing based on insolation values
    ...
    SUBROUTINE interpolation_with_insolation(mesh, ocean, time, grid_smooth)
      ! Linear interpolation forcing based on insolation values
    ...
    SUBROUTINE linear_time_interpolation(mesh, ocean, time)
      ...
      ! Safety Check: Ensure matrix timeframes are allocated
      IF (.NOT. ALLOCATED(ocean%matrix%timeframe0%T) .OR. .NOT. ALLOCATED(ocean%matrix%timeframe1%T) .OR. &
        .NOT. ALLOCATED(ocean%matrix%timeframe0%S) .OR. .NOT. ALLOCATED(ocean%matrix%timeframe1%S)) THEN
        CALL crash('Matrix timeframes are not allocated.')
      END IF
    ...
    SUBROUTINE interpolation_with_GHG_basic(mesh, ocean, time)
      ! Linear interpolation forcing based on GHG
    ...
    SUBROUTINE interpolation_with_GHG_radiative(mesh, ocean, time)
      ! Linear interpolation forcing based on GHG
    ...
    SUBROUTINE run_ocean_model_matrix( mesh, ice, ocean, time, region_name, grid_smooth)
      ! Calculate the ocean using an interpolating matrix ocean scheme
      ...
      CASE('linear_time')
        CALL linear_time_interpolation(mesh, ocean, time)
      CASE('GHG_radiative')
        CALL interpolation_with_GHG_radiative(mesh, ocean, time)
      CASE('GHG')
        CALL interpolation_with_GHG_basic(mesh, ocean, time)
      CASE('insolation')
        CALL interpolation_with_insolation(mesh, ocean, time, grid_smooth)
      CASE('d18O')
        CALL interpolation_with_d18O(mesh, ocean, time)
      CASE('anomaly_field')
        CALL apply_anomaly_fields(mesh, ocean, time)
      CASE('insolation+GHG')
        CALL interpolation_with_insolation_and_GHG(mesh, ocean, time, grid_smooth)
      CASE('insolation+GHG_radiative')
        CALL interpolation_with_insolation_and_GHG_radiative(mesh, ocean, time, grid_smooth)
      CASE DEFAULT
    ...
    SUBROUTINE initialise_ocean_model_matrix( mesh, ocean, region_name)
      ...
      CASE('insolation', 'insolation+GHG', 'insolation+GHG_radiative')
        ...
        CALL initialise_insolation_fields(mesh, ocean)
    ...
    SUBROUTINE initialise_insolation_fields(mesh, ocean)
