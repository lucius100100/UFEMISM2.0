MODULE main_regional_output
  ...
  ! =======================
    ...
    SUBROUTINE write_to_main_regional_output_file_grid_field( region, grid, filename, ncid, choice_output_field)
      ...
      ! Main ocean variables
        CASE ('T_ocean')
          CALL map_from_mesh_to_xy_grid_3D( region%mesh, grid, region%ocean%T, d_grid_vec_partial_3D)
          CALL write_to_field_multopt_grid_dp_3D( grid, filename, ncid, 'T_ocean', d_grid_vec_partial_3D)
        CASE ('S_ocean')
          CALL map_from_mesh_to_xy_grid_3D( region%mesh, grid, region%ocean%S, d_grid_vec_partial_3D)
          CALL write_to_field_multopt_grid_dp_3D( grid, filename, ncid, 'S_ocean', d_grid_vec_partial_3D)
    ...
    SUBROUTINE create_main_regional_output_file_grid_field( filename, ncid, choice_output_field)
      ...
      ! ==========================
        ...
        ! Main ocean variables
        CASE ('T_ocean')
          CALL add_field_grid_dp_3D( filename, ncid, 'T_ocean', long_name = 'Oceanic temperature', units = 'K')
        CASE ('S_ocean')
          CALL add_field_grid_dp_3D( filename, ncid, 'S_ocean', long_name = 'Oceanic salinity', units = 'PSU')
